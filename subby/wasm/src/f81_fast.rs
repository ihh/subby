...
/// Compute expected counts using closed-form F81 integrals, per branch.
/// Returns (R*A*A*C) flat array. Branch 0 = zeros.
pub fn f81_counts_per_branch(
    u: &[f64],
    d: &[f64],
    log_norm_u: &[f64],
    log_norm_d: &[f64],
        ...
        for col in 0..c {
            ...
            for i in 0..a {
                ...
                for j in 0..a {
                    ...
                    if i == j {
                        result[br_offset + i * a * c + j * c + col] = i_sum;
                    } else {
                        result[br_offset + i * a * c + j * c + col] = mu * pi[j] * i_sum;
                    }
                }
            }
        }
